...
namespace Pinocchio {
    ...
    class Vectorn : public std::vector<Real> {
        public:
            ...
            template<class R>
            Vectorn(const std::vector<R> &other) : super(other.begin(), other.end()) {}

            template<class Iter>
            Vectorn(const Iter &from, const Iter &to) : super(from, to) {}

            using super::size;

            //basic recursive functions
            template<class F>
            Vectorn<typename F::result_type> apply(const F &func) const {
            ...
            template<class F>
            Vectorn<typename F::result_type> apply(const F &func, const Self &other) const {
